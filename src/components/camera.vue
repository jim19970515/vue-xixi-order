...
<template>
    <div class="px-4 flex flex-col items-center gap-8 mt-16">
        <h1 class="text-center text-primary text-5xl font-extrabold" v-if="!destroyed">請掃描桌號</h1>
        <h1 class="text-center text-primary text-5xl font-extrabold" v-if="destroyed">掃瞄器關閉中</h1>
        <p>{{ error }}</p>
        {{ getCodes }}
        <h1 v-if="loading" class="text-primary text-3xl font-bold">loading...</h1>
        <div class="h-80 w-80 ">
            <qrcode-stream @error="onError" @detect="onDecode" @camera-on="onCameraOn" :formats="['qr_code']" v-if="!destroyed"></qrcode-stream>
        </div>
        <button @click="reload" v-if="!destroyed" class=" bg-primary px-8 py-2 text-4xl rounded-40 text-white font-medium">關閉</button>
        <button @click="reload" v-if="destroyed" class=" bg-primary px-8 py-2 text-4xl rounded-40 text-white font-medium">開啟</button>
    </div>
</template>
