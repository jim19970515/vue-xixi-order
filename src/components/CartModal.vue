  ...
  <div class="h-screen w-screen bg-white bg-opacity-50 backdrop-blur-sm z-50">
    <div class="flex flex-col m-4 p-4 bg-primary h-full overflow-hidden">
      ...
      <div
        ...
        class="overflow-y-auto max-h-full mb-auto mt-2"
      >
        <div
          class="bg-white p-2 mt-2 relative h-36"
          v-for="item in store.cartData.product"
          :key="item.id"
        >
          <div class="flex justify-between">
            ...
            <img
              ...
              class="object-cover w-32 h-32 rounded-20"
              alt=""
            />
            <button
              class="absolute bottom-2 right-2 text-white bg-primary px-2"
              @click="store.deleteCartData(item.id)"
            >
              刪除
            </button>
          </div>
      ...
      <div class="flex justify-between items-end py-2 h-[94px]">
        ...
        <button
          class="px-14 h-full bg-white text-primary text-3xl font-extrabold rounded-lg active:bg-secondary active:text-white duration-75"
          @click="goCheckOut"
        >
          結帳
        </button>
